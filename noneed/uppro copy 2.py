...
async def program():
    ...
    end = datetime.datetime(now.year, now.month, now.day) + datetime.timedelta(hours=14)
        ...
        while True:
            try:
                ...
                if end < now:
                    ...
                    krw = upbit.get_balance("KRW")
                    krw = round(krw)
                    end = datetime.datetime(now.year, now.month, now.day, now.hour) + datetime.timedelta(minutes=3)
                    logging.info(f"coin1 = {coin1}, coin1_percent = {coin1_percent}, coin1_open : {coin1_open}, coin1_ma10 : {coin1_ma10}, coin1_target : {coin1_target}, coin1_status : {coin1_status}")
                    logging.info(f"coin2 = {coin2}, coin2_percent = {coin2_percent}, coin2_open : {coin2_open}, coin2_ma10 : {coin2_ma10}, coin2_target : {coin2_target}, coin2_status : {coin2_status}")
                    logging.info(f"krw_balance : {krw}")
                ...
                # if coin2_current_price > 1 and coin2_status == 0 and coin2_open >= coin2_ma10:
                    try:
                        buy_limit(coin2, krw)
                        # print("buy coin2")
                        coin2_status = 1
